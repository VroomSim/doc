...
wget -N -q --show-progress "https://raw.githubusercontent.com/VroomSim/doc/main/SafeShutdown.py"
wget -N -q --show-progress "https://raw.githubusercontent.com/VroomSim/doc/main/multi_switch.sh"
...
cd /boot/
File=config.txt
if ! grep -q "^dtoverlay=gpio-poweroff,gpiopin=26,active_low=1,input=1" $File; then
    echo "Enable overlay file"
    echo "# Overlay setup for proper powercut, needed for Retroflag cases" >> "$File"
    echo "dtoverlay=gpio-poweroff,gpiopin=26,active_low=1,input=1" >> "$File"
fi
